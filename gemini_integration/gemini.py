frappe.pages['gemini-chat'].on_page_load = function(wrapper) {
    ...
    const add_to_history = (role, text, file_url = null) => {
        ...
        console.log("Adding to history:", { role, text, file_url });
        if (role === 'gemini') {
            console.log("showdown library status:", window.showdown);
        }
        // --- END OF FIX ---
    ...
    // --- THIS IS THE FIX ---
    // Use a function to load the script and ensure it's ready
    ...
    loadShowdown(() => {
        console.log("Showdown.js has been loaded.");
    });
    // --- END OF FIX ---
