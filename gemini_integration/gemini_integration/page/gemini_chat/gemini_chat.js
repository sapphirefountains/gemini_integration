	...
	const add_to_history = (role, text, image_url) => {
		...
		if (role === "user") {
			...
			if (frappe.session.user_image) {
				...
				const img = $("<img>").attr("alt", frappe.session.user_fullname);
				// Set error handler before setting the src to handle potential loading errors robustly.
