	...
	try:
		...
		results = (
			...
			.list(
				...
				orderBy="modifiedTime desc" if not q else None,
			)
			.execute()
		)
	...
	try:
		...
		def create_callback(msg_id):
			def callback(request_id, response, exception):
				if not exception:
					subject = next(
						(h["value"] for h in response["payload"]["headers"] if h["name"] == "Subject"),
						"No Subject",
					)
					email_data.append(
						{"id": msg_id, "subject": subject, "snippet": response.get("snippet", "")}
					)
...
def search_google_contacts(credentials, name_query, limit=5):
	...
	try:
		...
		results = (
			service.people()
			.searchContacts(query=name_query, pageSize=limit, readMask="names,emailAddresses")
			.execute()
		)
...
def get_drive_file_context(credentials, file_id):
	...
	try:
		...
		file_meta = (
			service.files()
			.get(fileId=file_id, fields="id, name, webViewLink, mimeType", supportsAllDrives=True)
			.execute()
		)
		...
		if "google-apps.document" in mime_type or "text/plain" in mime_type:
			...
			content_bytes = (
				...
				else service.files().get_media(fileId=file_id, supportsAllDrives=True).execute()
			)
		...
		frappe.log_error(
			f"Google Drive API Error for fileId {file_id}: {error.content}", "Gemini Google Drive Error"
		)
...
def search_documents(query):
	...
	for doctype in searchable_doctypes:
		matches, disclaimer = search_erpnext_documents(
			doctype, query, limit=100
		)  # Limit per doctype to avoid overload
		if matches:
...
def _generate_final_response(
	prompt, context, model, conversation_id, conversation_history, uploaded_files, generation_config=None
):
	...
	if context:
		final_prompt_parts.append(f"Context from ERPNext/Google:\n{context}")
...
def generate_chat_response(
	...
	if selected_options:
		...
		original_prompt = next(
			(entry["text"] for entry in reversed(conversation_history) if entry["role"] == "user"), prompt
		)
		return _generate_final_response(
			original_prompt,
			full_context,
			model,
			conversation_id,
		...
		if any(kw in prompt.lower() for kw in gmail_keywords):
			...
			if contact_name:
				...
				if len(contacts) == 1:
					...
					thoughts += (
						f"No Google Contact found for '{contact_name}'. Using full prompt for search.\n"
					)
	...
	if "clarification_options" in locals() and clarification_options:
		...
		return {
			"clarification_needed": True,
			"options": clarification_options,
			"response": "I found a few items that could match. Please select the correct one(s):",
			"thoughts": thoughts,
